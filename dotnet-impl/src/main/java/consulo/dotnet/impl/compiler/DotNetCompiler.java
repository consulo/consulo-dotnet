...
public class DotNetCompiler implements TranslatingCompiler {
    ...
    public boolean isCompilableFile(Path file, CompileContext context) {
        ...
        return FileTypeRegistry.getInstance().getFileTypeByFileName(file.getFileName().toString()) == extension.getFileType();
    }
    ...
    public void compile(CompileContext context, Chunk<Module> moduleChunk, Collection<Path> files, OutputSink sink) {
        ...
        try {
            GeneralCommandLine commandLine = builder.createCommandLine(module, files, dotNetModuleExtension);
            ...
            for (String line : processOutput.getStdoutLines()) {
                try {
                    ...
                    if (virtualFile != null && m.getCategory() == CompilerMessageCategory.ERROR) {
                        ...
                        if (problem != null) {
                            WolfTheProblemSolver.getInstance(module.getProject()).reportProblems(virtualFile, List.of(problem));
                        }
                    }

                    context.newMessage(m.getCategory(), LocalizeValue.of(m.getMessage()))
                        .optionalUrl(fileUrl)
                        .position(m.getLine(), m.getColumn())
                        .add();
                }
                catch (Exception e) {
                    LOGGER.error("Message with : " + line + " cant be parsed", e);
                    context.newError(LocalizeValue.of(line)).add();
                }
            }
            ...
            if (processOutput.getExitCode() != 0) {
                context.newError(LocalizeValue.localizeTODO("Exit code: " + processOutput.getExitCode())).add();
            }
        }
        catch (DotNetCompileFailedException e) {
            context.newError(LocalizeValue.ofNullable(e.getMessage())).add();
