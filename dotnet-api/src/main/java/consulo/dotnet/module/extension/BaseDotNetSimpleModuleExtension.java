...
public abstract class BaseDotNetSimpleModuleExtension<S extends BaseDotNetSimpleModuleExtension<S>> extends ModuleExtensionBase<S> implements DotNetSimpleModuleExtension<S> {
    ...
    protected Map<String, Map<String, String[]>> myUrlsCache = new HashMap<>();
    protected DotNetModuleSdkPointer mySdkPointer;

    public BaseDotNetSimpleModuleExtension(String id, ModuleRootLayer moduleRootLayer) {
    ...
    @Override
    public String[] getSystemLibraryUrls(String name, String orderRootType) {
        Sdk sdk = getSdk();
        if (sdk == null) {
        ...
        Map<String, String[]> orderRootTypeMap = myUrlsCache.get(name);
    ...
    @Nullable
    private File getLibraryByAssemblyName(final String name, @Nullable SimpleReference<Couple<String>> cache) {
    ...
    protected String[] getSystemLibraryUrlsImpl(Sdk sdk, String name, String orderRootType) {
        if (BinariesOrderRootType.ID.equals(orderRootType)) {
        ...
        else if (DocumentationOrderRootType.ID.equals(orderRootType)) {
            String[] systemLibraryUrls = getSystemLibraryUrls(name, BinariesOrderRootType.ID);
        ...
        else if (ExternalAttributesRootOrderType.ID.equals(orderRootType)) {
            try {
    ...
    private boolean isValidLibrary(File file, final String name, @Nullable SimpleReference<Couple<String>> cache) {
